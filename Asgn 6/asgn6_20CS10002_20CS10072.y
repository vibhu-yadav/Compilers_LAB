...
postfix_expression: 
        ...
        {   
            ...
            SymbolTable* funcTable = globalST.lookup($1->loc)->nestedST;
            emit($1->loc, "0", "", CALL);
        }
...
unary_expression: 
        ...
        {
            $$ = new expression();
            Type type = ST->lookup($2->loc)->type;
            if(type.type == ARRAY) {
...
and_expression: 
        ...
        | and_expression BITWISE_AND equality_expression
        {
            ...
            $$ = new expression();
            $$->loc = ST->gentemp();             
            emit($$->loc, $1->loc, $3->loc, LOGIC_AND);   
        }
        ;
...
exclusive_or_expression: 
        ...
        | exclusive_or_expression BITWISE_XOR and_expression
        {
            ...
            $$ = new expression();
            $$->loc = ST->gentemp();              
            emit($$->loc, $1->loc, $3->loc, LOGIC_XOR);    
        }
        ;
...
assignment_expression: 
        ...
        | unary_expression assignment_operator assignment_expression
        {
            ...
            if($1->fold == 0) {
                ...
                emit(*($1->folder), $3->loc, "", LEFT_DEREFERENCE);
            $$ = $1;       
        }
        ;
        ...
        declaration_specifiers init_declarator_list SEMICOLON
        {
            ...
            for(vector<declaration*>::iterator it = decs.begin(); it != decs.end(); it++) {
                ...
                if(currDec->type == FUNCTION) {
                    ...
                    Symbol* two = one->nestedST->lookup("RETVAL", currType, currDec->pointers);
                    one->size = 0;
                    one->initVal = NULL;
                    continue;
                }
...
function_prototype:
        declaration_specifiers declarator
        {
            ...
            if(currDec->type == FUNCTION) {
                Symbol* retval = sym->nestedST->lookup("RETVAL", currType, currDec->pointers); 
                sym->size = 0;
                sym->initVal = NULL;
            }
            $$ = $2;
